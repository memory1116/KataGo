...
int MainCmds::testgpuerror(const vector<string>& args) {
  ...
  try {
    ...
    TCLAP::SwitchArg requireExactNNLenArg(
      "", "require-exact-nn-len",
      "Require the configured neural-net dimensions exactly; useful for validating fixed-shape optimized backends"
    );
  ...
  if(requireExactNNLen)
    logger.write("Requiring exact neural-net dimensions for optimized fixed-shape backends");
  ...
  const bool defaultRequireExactNNLen = requireExactNNLen;
  ...
  }
  {
    if(nnEval->isAnyThreadUsingFP16()) {
      ...
      // Compute-only CUDA graph replay is an FP16 optimized-backend experiment.
      // Keep it enabled for the current configuration above, but disable it for
      // the absolute FP32 reference evaluator so the test compares graph output
      // against the ordinary reference path instead of tripping its capability gate.
      ConfigParser fp32Cfg(cfg);
      if(fp32Cfg.contains("cudaUseGraphInference"))
        fp32Cfg.overrideKey("cudaUseGraphInference","false");
      nnEval32 = Setup::initializeNNEvaluator(
        modelFile,modelFile,expectedSha256,fp32Cfg,logger,seedRand,expectedConcurrentEvals,
        defaultNNXSize,defaultNNYSize,maxBatchSize,defaultRequireExactNNLen,disableFP16,
        Setup::SETUP_FOR_BENCHMARK
      );
    }
